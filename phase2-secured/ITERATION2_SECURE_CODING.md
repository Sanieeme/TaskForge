...
practice. The concrete rules this project follows, all visible across
`src/` (see `ARCHITECTURE.md` for the full module map — the codebase is
organized by Separation of Concerns: config, data access, auth, validation,
output encoding, and each resource's controller each live in their own
file, rather than one large `server.js`):

| Standard | Applied where |
|---|---|
| Parameterised queries only, never string-built SQL | Every `db.prepare(...).run()/.get()/.all()` call across `src/controllers/*` |
| Validate all input at the boundary, reject early | `src/validation/validators.js`; length caps on titles/descriptions/comments in the relevant controller |
| Least privilege by default | `src/middleware/require-auth.js`'s `requireRole()` gate, called at the top of every privileged controller function; employees default to read/edit-own-task only |
| Escape untrusted output at the point of rendering | `src/utils/html.js`'s `escapeHtml()` applied server-side in `comments-controller.js`; `app.js` additionally uses `textContent`/DOM nodes instead of `innerHTML` (defense in depth — two independent layers, not one) |
| Fail closed, not open | `requireAuth`/`requireRole` return `401`/`403` and stop; nothing defaults to "allow" |
| No hardcoded secrets | `src/config.js` reads everything from `process.env`, with production refusing to start without `COOKIE_SIGNING_KEY` set |
| Single Responsibility per module | Each file in `src/` has exactly one reason to change — a password-hashing bug touches only `auth/password-utils.js`; an authorization bug touches only `middleware/require-auth.js` and its caller. This makes the standards above easier to *verify*, not just easier to state — see `ARCHITECTURE.md` for why this matters specifically for the access-control bugs found in Phase 1. |
