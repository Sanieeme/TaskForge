...
architecture (`server.js`, `db.js`, `public/app.js`) and to the 10 concrete
...
```
 ...
 │  - Route handlers (/api/login, /api/tasks, │
 │    /api/tasks/:id/comments, /api/users...) │
 └───────────────────────────────────────────┘
        │  db.prepare(...).run()/.get()/.all()
        ▼
 ┌───────────────────────────────────────────┐
 │  SQLite (taskforge.db) via db.js           │
...
| **R**epudiation | No audit trail of who changed a task's status or deleted it | B1/B3 | ✅ Yes (still open) | Not fully fixed in Phase 2 — see "Residual risk" below |
...
| **D**enial of Service | Oversized request bodies (e.g. a huge `avatar` string) consume memory/CPU with no limit | B1 | ✅ Yes | Body-size cap added in `readBody()` |
...
**Residual risk (honest gap, worth naming in a report):** Repudiation is not
fully addressed even in Phase 2 — there's no persistent audit log of
who-did-what-when. A production system would add an `audit_log` table
recording actor, action, target, and timestamp for every state-changing
request. Naming this kind of residual risk explicitly is itself good
security-engineering practice (NIST SSDF's "Respond to Vulnerabilities"
group, §4, expects ongoing identification of remaining gaps, not a claim of
total coverage).
...
| **Verification** | 0 — no testing beyond "does it run" | 1 — manual verification of each fix (see test output in project history); still missing automated tests |
| **Operations** | 0 — no patching/monitoring story | 0 — still not addressed; would require logging, alerting, dependency scanning in CI |

This is a legitimate SAMM-style finding: Phase 2 meaningfully improves
Design/Implementation/Verification maturity, but Operations remains
immature — a fair, honest limitation to state in a report rather than
claiming full maturity.
