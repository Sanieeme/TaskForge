...
// VULNERABLE (Phase 1): Server-Side Request Forgery (SSRF).
// This "attach a link preview" feature lets a user paste a URL; the server
// fetches it on their behalf and returns a snippet - a common real-world
// feature (Slack, Jira, and many task trackers do this). But there is
// ZERO validation of scheme or destination host. An attacker can point
// this at:
//   - internal-only services the server can reach but the attacker can't
//     (e.g. http://127.0.0.1:3000/api/users to dump this very app's own
//     admin data through the server's own trusted network position)
//   - cloud metadata endpoints (e.g. http://169.254.169.254/) in a real
//     cloud deployment, often used to steal IAM credentials
//   - internal ports for service/port scanning ("is redis running on
//     6379?") by timing the response
async function handleLinkPreview(req, res, taskId) {
  const session = getSession(req);
  if (!session) return sendJSON(res, 401, { error: "not logged in" });
  ...
  try {
    ...
    sendJSON(res, 502, { error: `could not fetch url: ${e.message}` });
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (m && req.method === "POST") return await handleLinkPreview(req, res, m[1]);
